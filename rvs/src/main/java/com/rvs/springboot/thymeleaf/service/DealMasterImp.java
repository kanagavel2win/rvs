...
public class DealMasterImp implements DealMasterService {
	...
	public void updatepipeline(String str, String pipeline, String notes) {
		...
		if (notes.equalsIgnoreCase("")) {
			...
			sql = "update dealmaster set pipeline ='" + pipeline + "', lossbacktodealreason= '" + notes.trim()
					+ "'  where id in (" + str + ")";
		}
	...
	@Override
	public int insertFiles(String DocumentType, String DocumentNo, String FilePath, int dealid, String createddate, String DocumentGroup) {
		final String sql = "INSERT INTO `dealfiles`(`document_no`, `document_type`, `file_path`, `id`, createddate,docgroup) VALUES ('"
				+ DocumentNo + "','" + DocumentType + "','" + FilePath + "'," + dealid + ",'" + createddate + "','" + DocumentGroup+"')";
	...
	public int insertContact(int contactpersonid, int dealid) {
		final String sql = "INSERT INTO deal_contact(`contact_person`, `id`) VALUES (" + contactpersonid + "," + dealid
				+ ")";
